...
Libris becomes a library of series: EPUB volumes, webnovel chapters in TXT files and JSON content sent by an automation API all go through the same pipeline, and later volumes learn from earlier ones without ever seeing later ones.

**Upgrading.** One database migration runs at start-up (`alembic upgrade head`, automatic in the Docker deployment). It groups existing books into series by owner and normalized series name, gives every EPUB a source-file row pointing at its existing file (nothing is read, moved or rewritten), and keeps every project, chapter and passage identifier: nothing is translated again. Things to know:
...
- New data directories: `DATA_DIR/sources` (TXT and JSON sources, back it up with the books — the scheduled backup already archives the whole volume) and `DATA_DIR/staging` (uploads waiting for confirmation, cleaned by the retention).
- New settings, all optional: `IMPORT_MAX_FILES`, `IMPORT_MAX_SESSION_MB`, `IMPORT_SESSION_HOURS`, `TEXT_CHAPTER_MAX_CHARS`, `API_MAX_PAYLOAD_MB`, `API_MAX_CHAPTERS`, `API_RATE_LIMIT_PER_MINUTE`.
...
- `alembic downgrade` to 0.5 keeps TXT and JSON volumes in the tables, but 0.5 cannot export them.
