...
def result_response(
    db, request_id: str, caller: Caller, requested: str | None, partial: bool, accept: str, scope: str = "request",
):  # fmt: skip
    found = owned_request(db, request_id, caller)
    refresh(db, found.id)
    ...
    stored_format = (found.artifact or {}).get("format") == fmt and scope == "request"
    content = stored(found) if stored_format else None
    if content is not None:
        filename = found.artifact.get("filename") or f"result.{fmt}"
    else:
        try:
            rendered = render(db, found, project, job, fmt, status, found.report, scope=scope)
...
async def translation_result(
    ...
    return await run_in_threadpool(result_response, db, request_id, caller, format, partial, accept, scope)
