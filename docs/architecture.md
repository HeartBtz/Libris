...
| `api_tokens` | Owner, name, SHA-256 of the secret, displayable prefix, scopes, expiry, revocation, last use, optional webhook signing secret (encrypted), optional cost budget (`budget_amount`, `budget_period`: `month` or `total`). |
...
| `app_settings` | Settings saved from the interface (autopilot, webhooks, OpenViking, SearXNG, provider recovery, budgets), watermarks and markers. |
