...
Register a dedicated shell runner on CT116 with the tag `libris-production-ct116`. Lock it to this project, mark it **protected**, disable untagged jobs, and configure it to accept only protected tags. The runner account needs local Docker access and passwordless sudo for exactly `/usr/local/sbin/libris-production-deploy`; install that file root-owned and non-writable by the runner. Do not run test, branch or merge request jobs on this deployment runner. Protect the `production` environment so only maintainers can start the manual job.
...
The protected manual `deploy-production` job is serialized by `resource_group` and can run only on the dedicated CT116 runner. It pulls the digest-pinned application and Codex images locally and passes their commit, version and immutable image IDs to the preinstalled root-owned deployment procedure. That fixed procedure gracefully stops the old API, worker and Codex bridge before creating a uniquely named PostgreSQL backup, preserves the existing named volumes and private `/opt/epub-translator/.env`, runs migrations, starts the API and Codex bridge, verifies the exact version through `/health`, and only then restarts the worker from its checkpoints. It also rejects an older version or a reused version number associated with another commit.
