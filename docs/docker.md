...
The installer creates `.env` with random secrets when needed, pulls `heartbtz/libris:0.5.0`, starts PostgreSQL, applies migrations, and waits for the API health check. It never replaces an existing `.env` or deletes volumes.
