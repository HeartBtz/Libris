...
Le chemin le plus court : configurer un provider (étape 1), puis **Ajouter du contenu** et **Importer et lancer tout le pipeline**. Le [pilote automatique](#pilote-automatique) mène alors le livre jusqu’au résultat, sans aucune validation à faire ; les étapes 4 à 7 ci-dessous restent possibles mais ne sont jamais nécessaires.
...
- **Pilote automatique** (livres suivis par le pilote) : rapport final, passages conservés en original avec leur raison, bouton de téléchargement du résultat et journal des décisions — voir [Pilote automatique](#pilote-automatique).
- **Journal des relectures** : les remarques, doutes et alertes relevés sur les passages, et les décisions de l’IA à leur sujet. Rien n’y attend de validation et rien n’empêche l’export : un passage peut être consulté, corrigé ou validé si on le souhaite ; une proposition peut être acceptée, refusée ou chargée dans l’éditeur avec **Modifier**. **Tout accepter** et **Lancer la revue IA** demandent confirmation.
...
Un livre lancé en entier (import avec pipeline, bouton **Lancer le pilote automatique**, requête d’API) va de sa source au résultat sans intervention ; le fonctionnement détaillé est décrit dans [autopilot.md](autopilot.md).

- **En cours** : un bandeau au-dessus des onglets indique la phase (récupération, cohérence, revue finale, arbitrage), le tour de convergence (« Tour 2 sur 3 ») et le fournisseur de secours utilisé après une panne. Rien n’est attendu de vous.
- **Terminé** : le bandeau propose aussitôt **Télécharger l’EPUB** (ou **Télécharger les chapitres** pour un volume texte) et **Voir le rapport** ; les autres formats restent dans **Exporter**. Un passage que personne n’a pu traduire garde son texte original dans le résultat, avec la raison.
- **Onglet Pilote automatique** : rapport final (tours, passages conservés en original, décisions consignées), liste des passages conservés (**Ouvrir le passage**, **Voir ses décisions**) et journal des décisions filtrable par étape ou par passage.
- **Réglages du livre** : suivre le réglage de l’installation, activer ou désactiver le pilote pour ce livre, et ordonner ses propres fournisseurs de secours (essayés avant ceux de l’installation).
- **Paramètres › Pilote automatique** (administrateurs) : valeurs de l’installation — pilote activé par défaut, tours de convergence, nombre et durée des attentes d’une panne, fournisseurs de secours ordonnés, seuils des décisions automatiques. Enregistrées, elles remplacent les variables `AUTOPILOT_*` aux décisions suivantes, sans redémarrage ; **Revenir aux valeurs de l’environnement** les oublie.

<img src="screenshots/autopilot-mobile.png" alt="Pilote automatique sur téléphone" width="390">
...
4. **Pré-analyse** : tableau modifiable (cartes sur téléphone). Le nom de série se corrige pour tout le lot et indique si la série existe déjà ; titres et numéros se modifient ; **Monter/Descendre** puis **Renuméroter dans cet ordre** fixent l’ordre (**Trier par numéro** pour les chapitres). Chaque numéro affiche sa confiance et sa raison ; un numéro de confiance faible est retenu automatiquement (« Numéro retenu automatiquement ; modifiable ici »), un volume sans numéro reçoit le suivant de la série, et l’écran final liste ces **Décisions automatiques** avec leur raison. Rien n’est à confirmer, sauf si l’installation l’exige (`IMPORT_CONFIRM_LOW_CONFIDENCE=true` : une case **Confirmer le numéro** réapparaît alors). Les numéros en double et les volumes déjà présents dans la série bloquent la suite ; les numéros manquants sont signalés. Les fichiers illisibles et les EPUB déjà présents dans la bibliothèque sont écartés. Pour les chapitres TXT : **Utiliser la première ligne comme titre**, et pour un chapitre déjà présent, « identique : ignoré » ou la case **Remplacer** à cocher explicitement. Si un remplacement devait supprimer des passages corrigés ou validés par une personne, une confirmation supplémentaire est demandée.
5. **Confirmation** : récapitulatif (série ou volumes uniques, volumes créés, chapitres créés, identiques, remplacés, fichiers ignorés) et réglages facultatifs (langues, provider, qualité, source de mémoire ; vides, ils reprennent les valeurs par défaut de la série). Trois choix : **Importer uniquement**, **Importer et lancer l’analyse**, et, en bouton principal, **Importer et lancer tout le pipeline** : le pilote automatique mène chaque volume jusqu’au résultat. Les avertissements (par exemple un provider manquant) et les refus du serveur s’affichent avec leur raison.
...
- **Envoi** : `POST /api/v1/translation-requests` avec `Authorization: Bearer <jeton>` et, au choix, un EPUB (fichier `file` en multipart avec les options en champs, ou corps `application/epub+zip` avec les options dans l’URL), des chapitres `.txt` (un ou plusieurs fichiers, numéros lus dans les noms), ou un JSON (ou un fichier `.json`) qui décrit la série, le volume, les langues (BCP 47), les chapitres (numéro, identifiant externe, titre, texte) et les options du pipeline. Le pipeline complet démarre par défaut ; aucun choix n’est demandé à une personne : un numéro incertain ou absent est décidé automatiquement et la raison est enregistrée. Tout est enregistré avant la réponse `202` ; l’en-tête `Idempotency-Key` ou le même `external_id` renvoie la même requête sans rien dupliquer (409 si le contenu diffère). Un chapitre déjà importé avec un autre texte est refusé sauf `replace_changed_chapters: true`.
- **Suivi et résultat** : la traduction tourne dans le worker ; si un travail occupe déjà le volume, la requête attend (`queued`) et démarre ensuite, même après un redémarrage. `GET …/{id}` donne l’état, l’étape et la progression ; `…/pause`, `…/resume`, `…/cancel` pilotent le travail ; `…/result` rend le résultat : l’EPUB traduit pour un EPUB (vérifié par EPUBCheck et réparé automatiquement au besoin), sinon `format=json|txt|txt-zip` (409 tant que la requête n’est pas terminée, sauf `partial=true` ; `?wait=60` attend la fin). Une requête finit toujours : `completed`, `completed_with_residuals` (quelques passages gardent leur texte source, listés avec leur raison), `failed` (avec la raison, y compris un travail resté bloqué trop longtemps) ou `cancelled`, avec un rapport (passages, résidus, tokens et coût, durées).
- **Webhook** (facultatif) : `callback_url` n’est accepté que vers un hôte autorisé par l’administrateur, jamais vers une adresse privée non autorisée ; l’appel est signé (HMAC-SHA256, en-tête `X-Libris-Signature`), envoyé par le worker et réessayé quelques fois. Un jeton peut recevoir son propre secret de signature à sa création (case **Signer les webhooks avec un secret propre à ce jeton**, affiché une seule fois). Les hôtes et réseaux autorisés, les tentatives, le délai et le secret global se règlent dans *Paramètres › API d’automatisation › Webhooks des requêtes d’API* (sinon `API_WEBHOOK_*`) ; le secret n’est jamais réaffiché.
- **Limites** : taille (`API_MAX_PAYLOAD_MB`), nombre de chapitres (`API_MAX_CHAPTERS`) et débit par jeton (`API_RATE_LIMIT_PER_MINUTE`, compté par processus de l’API).
