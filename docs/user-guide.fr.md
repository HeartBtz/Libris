...
- **Webhook** (facultatif) : `callback_url` n’est accepté que vers un hôte autorisé par l’administrateur (`API_WEBHOOK_HOSTS`), jamais vers une adresse privée non autorisée ; l’appel est signé (HMAC-SHA256), envoyé par le worker et réessayé quelques fois.
